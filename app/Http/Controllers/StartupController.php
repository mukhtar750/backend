...
class StartupController extends Controller
{
    ...
    /**
     * Display access requests for the entrepreneur's startups
     *
    ...
    public function accessRequests()
    {
        ...
        $startups = Startup::where('founder_id', Auth::id())->pluck('id');
        
        // Get all access requests for these startups
        $accessRequests = \App\Models\StartupAccessRequest::whereIn('startup_id', $startups)
            ->with(['investor', 'startup'])
            ->orderBy('created_at', 'desc')
            ->get();
        
        return view('entrepreneur.access-requests', compact('accessRequests'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function approveAccess($requestId)
    {
        $accessRequest = \App\Models\StartupAccessRequest::findOrFail($requestId);
        
        // Check if the user owns the startup
        $startup = Startup::findOrFail($accessRequest->startup_id);
        if (Auth::id() !== $startup->founder_id) {
            return redirect()->back()->with('error', 'You are not authorized to manage this access request.');
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function rejectAccess(Request $request, $requestId)
    {
        $accessRequest = \App\Models\StartupAccessRequest::findOrFail($requestId);
        ...
        $accessRequest->response_message = $request->response_message;
        $accessRequest->save();
     ...
     * @return \Illuminate\Http\Response
     */
    public function revokeAccess($requestId)
    {
        $accessRequest = \App\Models\StartupAccessRequest::findOrFail($requestId);
        ...
        // Update the request status
        $accessRequest->status = 'rejected';
        $accessRequest->response_message = 'Access revoked by startup founder.';
        $accessRequest->save();
        
        // Notify the investor
        $accessRequest->investor->notify(new \App\Notifications\AccessRevoked($accessRequest));
