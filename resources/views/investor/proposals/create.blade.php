...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Create Pitch Event Proposal</h1>
            <p class="text-gray-600 mt-2">Propose a new pitch event for our platform. Your proposal will be reviewed by our admin team.</p>
        </div>
        ...
        <form action="{{ route('investor.proposals.store') }}" method="POST" class="bg-white rounded-lg shadow-lg p-8">
            @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="event_type" id="event_type" required
                            ...
                            <option value="pitch_competition" {{ old('event_type') == 'pitch_competition' ? 'selected' : '' }}>Pitch Competition</option>
                            <option value="networking" {{ old('event_type') == 'networking' ? 'selected' : '' }}>Networking Event</option>
                            <option value="demo_day" {{ old('event_type') == 'demo_day' ? 'selected' : '' }}>Demo Day</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="target_sector" id="target_sector" value="{{ old('target_sector') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <select name="target_stage" id="target_stage"
                            ...
                            <option value="seed" {{ old('target_stage') == 'seed' ? 'selected' : '' }}>Seed Stage</option>
                            <option value="series_a" {{ old('target_stage') == 'series_a' ? 'selected' : '' }}>Series A</option>
                            <option value="series_b" {{ old('target_stage') == 'series_b' ? 'selected' : '' }}>Series B</option>
                            <option value="series_c" {{ old('target_stage') == 'series_c' ? 'selected' : '' }}>Series C</option>
                            <option value="growth" {{ old('target_stage') == 'growth' ? 'selected' : '' }}>Growth Stage</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="target_criteria" id="target_criteria" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="Specific criteria for participating startups...">{{ old('target_criteria') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="min_funding_needed" id="min_funding_needed" value="{{ old('min_funding_needed') }}" min="0" step="1000"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="max_funding_needed" class="block text-sm font-medium text-gray-700 mb-2">Max Funding Needed ($)</label>
                            <input type="number" name="max_funding_needed" id="max_funding_needed" value="{{ old('max_funding_needed') }}" min="0" step="1000"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="expected_outcomes" id="expected_outcomes" rows="3" required
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="What do you expect to achieve with this event?">{{ old('expected_outcomes') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="success_metrics" id="success_metrics" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="How will you measure the success of this event?">{{ old('success_metrics') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 space-y-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="proposed_format" id="proposed_format" rows="4"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                      placeholder="Detailed description of the event format, agenda, and structure...">{{ old('proposed_format') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="supporting_rationale" id="supporting_rationale" rows="4"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                      placeholder="Why is this event needed? What market gap does it address?">{{ old('supporting_rationale') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="proposed_date" id="proposed_date" value="{{ old('proposed_date') }}"
                                       min="{{ date('Y-m-d', strtotime('+1 day')) }}"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label for="proposed_time" class="block text-sm font-medium text-gray-700 mb-2">Proposed Time</label>
                                <input type="time" name="proposed_time" id="proposed_time" value="{{ old('proposed_time') }}"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="proposed_duration" id="proposed_duration" value="{{ old('proposed_duration') }}" min="30" max="480"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>

                        <div>
                            <label for="proposed_location" class="block text-sm font-medium text-gray-700 mb-2">Proposed Location</label>
                            <input type="text" name="proposed_location" id="proposed_location" value="{{ old('proposed_location') }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Physical location or 'Virtual'">
                        </div>

                        <div class="flex items-center">
                            <input type="checkbox" name="is_virtual" id="is_virtual" value="1" {{ old('is_virtual') ? 'checked' : '' }}
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                            <label for="is_virtual" class="ml-2 block text-sm text-gray-900">Virtual Event</label>
                        </div>

                        <div id="virtual_platform_div" class="{{ old('is_virtual') ? '' : 'hidden' }}">
                            <label for="virtual_platform" class="block text-sm font-medium text-gray-700 mb-2">Virtual Platform</label>
                            <input type="text" name="virtual_platform" id="virtual_platform" value="{{ old('virtual_platform') }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <textarea name="additional_requirements" id="additional_requirements" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Any additional requirements, special needs, or considerations...">{{ old('additional_requirements') }}</textarea>
                </div>
            </div>
