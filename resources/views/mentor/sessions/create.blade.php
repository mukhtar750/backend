...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="bg-white rounded-xl shadow p-8 mb-8" x-data="scheduleSessionForm()">
        <form @submit.prevent="submitForm()">
            <!-- Mentee Selection -->
            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Select Mentee/Entrepreneur: <span class="text-red-500">*</span></label>
                <select x-model="formData.mentee_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" required>
                    <option value="">Choose a mentee...</option>
                    @foreach($pairedMentees as $mentee)
                        <option value="{{ $mentee->id }}">
                            <img src="{{ $mentee->profile_photo_url ?? asset('images/avatar-placeholder.png') }}" alt="Mentee" class="h-8 w-8 rounded-full object-cover mr-2">
                            {{ $mentee->name }} - {{ $mentee->business_name ?? $mentee->startup_name ?? 'User' }}
                        </option>
                    @endforeach
                </select>
                <p class="text-sm text-gray-500 mt-1">You can only schedule sessions with your paired mentees or entrepreneurs</p>
            </div>
            <!-- Session Title/Topic -->
            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Session Topic: <span class="text-red-500">*</span></label>
                <input type="text" x-model="formData.topic" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" placeholder="e.g., Pitch Practice, Business Model, Funding" required>
            </div>
            <!-- Session Type -->
            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Session Type: <span class="text-red-500">*</span></label>
                <select x-model="formData.session_type" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" required>
            ...
            <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" x-model="formData.session_date" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" required>
                </div>
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Session Time: <span class="text-red-500">*</span></label>
                    <input type="time" x-model="formData.session_time" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" required>
                </div>
            </div>
            <!-- Session Link (Optional) -->
            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Session Link (Optional):</label>
                <input type="url" x-model="formData.session_link" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" placeholder="https://meet.google.com/... or Zoom link">
            ...
            <div class="mb-6">
                ...
                <textarea x-model="formData.notes" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" placeholder="Additional details about the session, agenda, or preparation needed..."></textarea>
            </div>
            <!-- Submit Button -->
            <div class="flex items-center justify-end space-x-4">
                ...
                <button type="submit" :disabled="submitting" class="bg-indigo-700 text-white px-6 py-2 rounded-lg font-semibold shadow hover:bg-indigo-800 transition disabled:opacity-50">
        ...
        <div class="space-y-4">
            ...
            @empty
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-500">Schedule your first session with a mentee or entrepreneur to get started.</p>
                </div>
            @endforelse
        </div>
    </div>
...
function scheduleSessionForm() {
    return {
        ...
        async submitForm() {
            ...
            try {
                const response = await fetch('{{ route('mentor.sessions.create') }}', {
                    method: 'POST',
                    headers: {
